...
import {
  ...
  trocarCodigoPorToken, inscreverAppNaWaba, registrarNumero,
} from '@/lib/meta'
  ...
  try {
    ...
    return Response.json({
      ...
      conectado: atual?.config?.waba_id
        ? { waba_id: atual.config.waba_id, phone_number_id: atual.config.phone_number_id, origem: atual.config.origem || 'manual' }
        : null,
    })
  } catch (err: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const phoneId = soDigitos(body.phone_number_id)
    ...
    if (!phoneId) return apiError(400, 'A Meta não devolveu o número (phone_number_id). Refaça a conexão escolhendo um número.')
    ...
    return Response.json({ ok: true, waba_id: wabaId, phone_number_id: phoneId, passos })
  } catch (err: any) {
    return apiError(err.status || 500, err.message || 'Erro interno.')
  }
}
