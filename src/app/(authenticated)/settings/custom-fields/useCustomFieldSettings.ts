...
export function useCustomFieldSettings() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(BASE_PATH + '/api/custom-fields')
  ...
  const updateCategory = async (id: string, updates: Partial<CustomFieldCategory>) => {
    const res = await fetch(`${BASE_PATH}/api/custom-fields/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deleteCategory = async (id: string) => {
    const res = await fetch(`${BASE_PATH}/api/custom-fields/${id}?type=category`, { method: 'DELETE' })
  ...
  const updateFieldRanks = async (updates: { id: string; rank: number }[]) => {
    ...
    await Promise.all(updates.map(u =>
      fetch(`${BASE_PATH}/api/custom-fields/${u.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteField = async (id: string) => {
    const res = await fetch(`${BASE_PATH}/api/custom-fields/${id}`, { method: 'DELETE' })
