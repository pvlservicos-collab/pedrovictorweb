...
export default function ZapiPage() {
  ...
  const webhookUrl = organizationId
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}${BASE_PATH}/api/webhooks/zapi?org_id=${organizationId}`
    : ''

  const carregar = useCallback(async () => {
    setCarregando(true)
    try {
      const res = await fetch(BASE_PATH + '/api/integrations/zapi')
  ...
  async function importarConversas() {
    ...
    try {
      for (let volta = 0; volta < 40; volta++) {
        const res = await fetch(BASE_PATH + '/api/integrations/zapi/chats', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
