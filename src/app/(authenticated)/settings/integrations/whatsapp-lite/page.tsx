...
export default function WhatsAppLitePage() {
    ...
    useEffect(() => {
        const fetchOrgDetails = async () => {
            if (organizationId) {
                const orgRes = await fetch(BASE_PATH + '/api/organizations')
    ...
    const saveIntegrationToDb = async (name: string, token: string) => {
        ...
        try {
            ...
            const existingRes = await fetch(BASE_PATH + '/api/integrations?name=WhatsApp+Lite')
    ...
    const initializeInstance = async (name: string) => {
        ...
        if (organizationId) {
            const dbRes = await fetch(BASE_PATH + '/api/integrations?name=WhatsApp+Lite')
