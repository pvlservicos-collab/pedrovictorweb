...
export default function WhatsAppEvolutionPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrg = async () => {
      const res = await fetch(BASE_PATH + '/api/organizations')
  ...
  const loadIntegration = async () => {
    ...
    const res = await fetch(`${BASE_PATH}/api/integrations?name=${encodeURIComponent(INTEGRATION_NAME)}`)
  ...
  const saveIntegration = async (cfg: Record<string, any>) => {
    ...
    if (existing) {
      await fetch(BASE_PATH + '/api/integrations', {
        method: 'PATCH',
  ...
  useEffect(() => {
    ...
    const webhookUrl = `${window.location.origin}${BASE_PATH}/api/webhooks/evolution?org_id=${organizationId}`
