...
function AtivarContaForm() {
    ...
    useEffect(() => {
        ...
        let cancelled = false
        fetch(`${BASE_PATH}/api/auth/setup-owner?token=${encodeURIComponent(token)}`)
            .then((res) => res.json())
            .then((data) => {
                if (cancelled) return
                if (data?.valid) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setSubmitting(true)
        try {
            const res = await fetch(BASE_PATH + '/api/auth/setup-owner', {
                method: 'POST',
    ...
    return (
        <div className="min-h-screen bg-void relative overflow-hidden flex flex-col items-center justify-center p-4">
            <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
                <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[700px] h-[700px] rounded-full bg-[radial-gradient(circle,rgba(26, 184, 255,0.14)_0%,transparent_70%)]" />
            </div>

            <div className="relative w-full max-w-sm">
                <div className="text-center mb-8">
                    <img
                        src={`${BASE_PATH}/logos/pvlcrm-simbolo.svg`}
                        alt="PVL CRM"
                        className="w-20 h-20 mx-auto mb-4 object-contain drop-shadow-[0_0_28px_rgba(26, 184, 255,0.35)]"
                    />
