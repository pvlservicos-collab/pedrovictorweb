...
export default function OrgDetailModal({ organizationId, onClose, onUpdated }: {
  ...
  useEffect(() => { if (typeof window !== 'undefined') setOrigin(window.location.origin + BASE_PATH) }, [])

  const load = () => {
    setLoading(true)
    fetch(`${BASE_PATH}/api/admin/organizations/${organizationId}`)
      .then(async (res) => {
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    setResetResult(null)
    try {
      const res = await fetch(`${BASE_PATH}/api/admin/users/${userId}/reset-password`, { method: 'POST' })
      const data = await res.json()
