...
export default function LeadList({
  ...
  const markLeadAsRead = useCallback((leadId: string) => {
    ...
    if (lead?.is_unread && !jaMarcados.current.has(leadId)) {
      ...
      fetch(`${BASE_PATH}/api/leads/${lead.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_unread: false }) })
    }
  }, [leads, onUpdateLead])
  ...
  const handleTogglePin = useCallback(async () => {
    ...
    try {
      await fetch(`${BASE_PATH}/api/leads/${lead.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_pinned: newPinned }) })
    } catch (err) {
  ...
  const handleToggleArchive = useCallback(async () => {
    ...
    try {
      await fetch(`${BASE_PATH}/api/leads/${lead.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_archived: newArchived }) })
    } catch (err) {
