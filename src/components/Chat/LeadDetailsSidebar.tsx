...
export default function LeadDetailsSidebar({
  ...
  const handleAssignOwner = async (memberId: string | null) => {
    ...
    setSavingOwner(true)
    try {
      const res = await fetch(`${BASE_PATH}/api/leads/${lead.id}`, {
        method: 'PATCH',
  ...
  const handleSaveName = async () => {
    ...
    if (!res.ok) {
      ...
      await fetch(`${BASE_PATH}/api/leads/${lead.id}/activities`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: 'system',
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
      const res = await fetch(`${BASE_PATH}/api/leads/${lead.id}/messages`, { method: 'DELETE' })
      if (!res.ok) {
