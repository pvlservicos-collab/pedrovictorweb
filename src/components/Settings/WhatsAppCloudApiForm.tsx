...
export default function WhatsAppCloudApiForm({ onConnected, compact = false }: WhatsAppCloudApiFormProps) {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') setOrigin(window.location.origin + BASE_PATH)
        fetch(BASE_PATH + '/api/integrations/whatsapp-cloud/verify-token')
            .then((res) => res.json())
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const res = await fetch(`${BASE_PATH}/api/integrations/whatsapp-cloud?organization_id=${organizationId}`)
