...
// O CRM mora em /crm (basePath do Next). O servidor manda links como
// '/chat?leadId=...' (src/lib/push.ts); aqui eles ganham o prefixo.
const BASE = '/crm'
const comBase = (u) => (u.startsWith('/') && !u.startsWith(BASE + '/') ? BASE + u : u)
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title || 'Nova mensagem', {
      ...
      renotify: true,
      data: { url: comBase(payload.url || '/chat') },
    })
  )
})
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = comBase(event.notification.data?.url || '/chat')
