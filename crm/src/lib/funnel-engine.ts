 ...
 *
 * A ordem importa: NEXT_PUBLIC_APP_URL e o endereco publico (com o /crm).
 * VERCEL_PROJECT_PRODUCTION_URL e o dominio estavel do projeto, VERCEL_URL e o
 * do deploy especifico (muda a cada push, serve pra preview).
 *
 * O ultimo fallback e o endereco publico deste projeto — antes apontava pro
...
export function getBaseUrl() {
  // NEXT_PUBLIC_APP_URL vem primeiro agora: o dominio de producao do projeto na
  // Vercel e o *.vercel.app que so o proxy do site enxerga, e esses links vao
  // pro cliente. O retorno ja inclui o /crm (basePath), e quem chama so
  // acrescenta a rota.
  if (process.env.NEXT_PUBLIC_APP_URL) return process.env.NEXT_PUBLIC_APP_URL.replace(/\/+$/, '')
  if (process.env.VERCEL_PROJECT_PRODUCTION_URL) return `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}${BASE_PATH}`
  if (process.env.VERCEL_URL) return `https://${process.env.VERCEL_URL}${BASE_PATH}`
  return `https://pedrovictorweb.com.br${BASE_PATH}`
}
