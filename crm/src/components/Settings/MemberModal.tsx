...
export default function MemberModal({ organizationId, member, onClose }: MemberModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const endpoint = isEditing ? `${BASE_PATH}/api/users/${member.id}` : BASE_PATH + '/api/users'
  ...
  const handleDelete = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`${BASE_PATH}/api/users/${member.id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } })
      const result = await res.json()
