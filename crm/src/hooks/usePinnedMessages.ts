...
export function usePinnedMessages(leadId: string) {
  ...
  const fetchPinned = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE_PATH}/api/leads/${leadId}/pinned-messages`)
  ...
  const togglePin = useCallback(async (activity: LeadActivityWithActor) => {
    ...
    try {
      if (isPinned) {
        ...
        await fetch(`${BASE_PATH}/api/leads/${leadId}/pinned-messages/${activity.id}`, { method: 'DELETE' })
      } else {
        ...
        await fetch(`${BASE_PATH}/api/leads/${leadId}/pinned-messages`, {
          method: 'POST',
