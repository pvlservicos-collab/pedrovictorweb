...
function fetchPipelinesDeduped(organizationId: string): Promise<any> {
  ...
  const promise = fetch(BASE_PATH + '/api/pipelines')
    .then(res => {
...
export function usePipeline(organizationId: string) {
  ...
  async function fetchStages(pipelineId: string) {
    try {
      const res = await fetch(`${BASE_PATH}/api/pipelines/${pipelineId}/stages`)
      if (!res.ok) return
  ...
  async function createPipeline(name: string) {
    try {
      const res = await fetch(BASE_PATH + '/api/pipelines', {
        method: 'POST',
  ...
  async function updatePipeline(pipelineId: string, updates: Partial<Pipeline>) {
    try {
      const res = await fetch(`${BASE_PATH}/api/pipelines/${pipelineId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function deletePipeline(pipelineId: string) {
    try {
      const res = await fetch(`${BASE_PATH}/api/pipelines/${pipelineId}`, { method: 'DELETE' })
      if (!res.ok) {
  ...
  async function updateStage(stageId: string, pipelineId: string, updates: Partial<PipelineStage>) {
    try {
      const res = await fetch(`${BASE_PATH}/api/pipelines/stages/${stageId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updates),
      })
  ...
  async function deleteStage(stageId: string, pipelineId: string) {
    try {
      const res = await fetch(`${BASE_PATH}/api/pipelines/stages/${stageId}`, { method: 'DELETE' })
      if (!res.ok) {
        const { error } = await res.json()
  ...
  async function reorderStages(pipelineId: string, newStages: PipelineStage[]) {
    ...
    try {
      await Promise.all(
        newStages.map(stage =>
          fetch(`${BASE_PATH}/api/pipelines/stages/${stage.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
