...
export default function ProductsSettingsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true)
    try {
      const res = await fetch(BASE_PATH + '/api/products?include_inactive=true')
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res: Response
      if (editingProduct) {
        res = await fetch(`${BASE_PATH}/api/products/${editingProduct.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      } else {
        res = await fetch(BASE_PATH + '/api/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      }
      if (!res.ok) {
        const data = await res.json()
  ...
  const handleDelete = async (product: Product) => {
    ...
    try {
      await fetch(`${BASE_PATH}/api/products/${product.id}`, { method: 'DELETE' })
  ...
  const handleToggleStatus = async (product: Product) => {
    ...
    try {
      await fetch(`${BASE_PATH}/api/products/${product.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }) })
    } catch {
