...
export default function ChatEvolutionPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_PATH}/api/integrations?name=${encodeURIComponent(INTEGRATION_NAME)}`)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      const res = await fetch(`${BASE_PATH}/api/leads/${leadIdFromUrl}`)
      if (res.ok) {
  ...
  const handlePipelineChange = useCallback(async (newPipelineId: string) => {
    ...
    try {
      ...
      if (!firstStageId) {
        const stagesRes = await fetch(`${BASE_PATH}/api/pipelines/${newPipelineId}/stages`)
  ...
  const handleChatMessageSent = useCallback((content: string) => {
    ...
    if (leadId && memberId && !alreadyHasOwner) {
      fetch(`${BASE_PATH}/api/leads/${leadId}`, {
        method: 'PATCH',
